  ...
  const getFilteredData = (baseData: any, filterType: string = "default") => {
    ...
    if (roleFilter !== "all" && filterType === "competency") {
      ...
      filteredData = filteredData.map((item) => ({
        ...item,
        [competencyFilter]: Math.max(
          60,
          Math.min(
            95,
            item[competencyFilter] +
              (roleFilter === "engineer"
                ? 5
                : roleFilter === "manager"
                  ? -3
    ...
    if (filterType === "timeline" || filterType === "interviews") {
      filteredData = filteredData.map((item) => ({
        ...
        ...(item.highlyRecommended && {
          highlyRecommended: Math.floor(
            item.highlyRecommended * timeMultiplier,
          ),
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <Icon
                  className={`w-5 h-5 ${kpi.isNegative ? "text-red-500" : "text-primary"}`}
                />
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="space-y-1">
            {filteredCompetencyData
              .sort(
                (a, b) =>
                  b[competencyFilter as keyof typeof a] -
                  a[competencyFilter as keyof typeof a],
              )
              .map((item) => {
                ...
                return (
                  <div
                    key={item.competency}
                    className={`flex items-center justify-between p-2 border rounded ${getScoreColor(score)}`}
                  >
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full border ${getPriorityStyle(alert.priority)}`}
                      >
