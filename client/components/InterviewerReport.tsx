...
export default function InterviewerReport({
  onInterviewerSelect,
}: InterviewerReportProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [sortBy, setSortBy] = useState("name");
  const [viewMode, setViewMode] = useState<"card" | "list">("card");
  const [topRecruiterDepartmentFilter, setTopRecruiterDepartmentFilter] =
    useState("all");
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-md font-semibold text-foreground">
              🏆 Top Recruiters - Hires
            </h4>
            <div className="flex space-x-2">
              <select
                value={topRecruiterDepartmentFilter}
                onChange={(e) =>
                  setTopRecruiterDepartmentFilter(e.target.value)
                }
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {topRecruiterDepartmentFilter === "all"
                      ? "All Departments"
                      : topRecruiterDepartmentFilter.charAt(0).toUpperCase() +
                        topRecruiterDepartmentFilter.slice(1)}{" "}
                    |{" "}
                    {topRecruiterRoleFilter === "all"
                      ? "All Roles"
                      : topRecruiterRoleFilter
                          .split("-")
                          .map(
                            (word) =>
                          ...
                          .join(" ")}{" "}
                    - {interviewer.avg_score}% avg score
                  </p>
                </div>
              </div>
            ))}
